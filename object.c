...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    // TODO: Implement
    // Step 1: Build header: "blob 16\0" / "tree 16\0" / "commit 16\0"
    const char *type_str;
    ...
    // Step 2: Combine header + data into one buffer and hash it
    ...
    // The shard dir is everything up to the last '/'
    ...
    *slash = '\0';
    ...
    free(full);
    if (written != (ssize_t)total) { close(fd); unlink(tmp_path); return -1; }

    // Step 6: fsync the temp file
    if (fsync(fd) != 0) { close(fd); unlink(tmp_path); return -1; }
    close(fd);

    // Step 7: Atomically rename temp -> final path
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    uint8_t *data = malloc(data_len + 1);  // +1 for safety null terminator
